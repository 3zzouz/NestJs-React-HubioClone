...
function JoinStudent() {
  ...
  const signup = () => {
    ...
    formData.append("level", inputValue.level);
    axiosapi
      .post("http://localhost:5000/users", formData)
      .then((res) => {
        console.log(res.data.data, "res ******");
        alert("Student " + res.data.data.userName + " created successfully");
        navigator('/login')
      })
      .catch((error) => {
      ...
      <div className="flex flex-row-reverse  xl:max-h-[100vh] xl:overflow-hidden ">
        ...
        <div className="mx-auto mt-10 flex w-[95%] flex-col items-center space-y-16 xl:min-w-[50vw] xl:space-y-7">
          ...
          <div className="flex w-[100%] flex-col items-center">
            ...
            <div className="mx-auto my-5 flex w-[90%] items-center justify-center space-x-3 md:justify-start">
              <input
                type="checkbox"
                className="h-4 w-4"
                onClick={(e) => setShowError((prev) => !prev)}
              />
              <h3>
                I accept the
                <span
